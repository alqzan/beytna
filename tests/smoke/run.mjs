...
try {
  ...
  const mapOverlay = pageA.locator('.overlay.world-map-modal')
  const mapViewBeforeZoom = await mapOverlay.locator('.world-map-svg').getAttribute('viewBox')
  await mapOverlay.locator('.world-map-expand').click()
  await pageA.waitForSelector('.overlay.world-map-modal.is-expanded', { timeout: 5000 })
  if (await mapOverlay.locator('.world-map-expand').getAttribute('aria-pressed') !== 'true') fail('world map expand button did not enter full-screen mode')
  await mapOverlay.locator('.world-map-zoom-btn').first().click()
  const mapViewAfterZoom = await mapOverlay.locator('.world-map-svg').getAttribute('viewBox')
  if (mapViewBeforeZoom === mapViewAfterZoom) fail('world map zoom control did not change the viewBox')
  const mapBox = await mapOverlay.locator('.world-map-svg').boundingBox()
  await pageA.mouse.move(mapBox.x + mapBox.width * .5, mapBox.y + mapBox.height * .5)
  await pageA.mouse.down()
  await pageA.mouse.move(mapBox.x + mapBox.width * .6, mapBox.y + mapBox.height * .5)
  await pageA.mouse.up()
  const mapViewAfterPan = await mapOverlay.locator('.world-map-svg').getAttribute('viewBox')
  if (mapViewAfterZoom === mapViewAfterPan) fail('world map drag did not move the view')
  await mapOverlay.locator('.world-map-zoom-reset').click()
  if (await mapOverlay.locator('.world-map-svg').getAttribute('viewBox') !== mapViewBeforeZoom) fail('world map reset control did not restore the full view')
  ...
  await pageA.locator('.dashboard-action').filter({ hasText: 'مهمة' }).click()
  await pageA.click('button:has-text("+ إضافة مهمة")')
  await pageA.fill('input[placeholder="مثلاً: حجز موعد الصيانة"]', 'تأكيد خطة السفر')
  await pageA.locator('.overlay').last().locator('button.btn-primary').click()
  await pageA.locator('.overlay').last().locator('.sheet-x').click()
  await pageA.locator('.dashboard-action').filter({ hasText: 'تذكير' }).click()
  await pageA.click('button:has-text("+ إضافة تذكير")')
