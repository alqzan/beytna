...
export async function parseBackup(text) {
  ...
  // اقبل النسخة الخام التي تصدرها أداة الهجرة أيضاً، حتى لا يعتمد الاستيراد
  // على اختيار الملف الثاني بالاسم الصحيح فقط. نتحقق من checksum قبل تحويلها.
  if (json.schemaVersion === 'legacy-raw-1') {
    if (!json.raw || typeof json.raw !== 'object' || Array.isArray(json.raw)) {
      throw new Error('النسخة الخام لا تحتوي بيانات صالحة')
    }
    const rawChecksum = await sha256Hex(JSON.stringify(json.raw))
    if (rawChecksum !== json.checksum) throw new Error('النسخة الخام تالفة أو مُعدَّلة — checksum غير مطابق')
    return buildBackup({
      sections: {
        rules: json.raw.rules || [],
        travel: json.raw.travel || [],
        packing: json.raw.packing || [],
      },
      prefs: json.raw.prefs || {},
    }, json.raw.shopping || [], json.familyId || 'beytna-legacy')
  }
